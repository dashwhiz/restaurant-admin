...
export async function importDaily(date: string, items: DailySaleInput[], deductStock: boolean): Promise<number> {
  ...
  for (const it of items) {
    const q = Math.round(it.qty); // one rounded qty for both the sale and the deduction
    const { error } = await sb.from('sales').insert({ recipe_id: it.recipeId, quantity: q, notes: `POS ${date}` });
    if (error) throw error;
    if (deductStock) await applyRecipeStock(it.recipeId, q);
    count++;
  }
  return count;
}
...
export async function getMappings(): Promise<PosMapping[]> {
  ...
  if (error) {
    if (TABLE_MISSING.test(error.message)) return []; // table not created yet
    throw error;
  }
  return (data ?? []) as PosMapping[];
}
export async function saveMapping(sifra: string, recipe_id: string | null, skip: boolean): Promise<void> {
  const { error } = await getSupabase().from('pos_mappings').upsert({ sifra, recipe_id, skip }, { onConflict: 'sifra' });
  if (error && !TABLE_MISSING.test(error.message)) throw error;
}
